...
const PlanSection = () => {
  ...
  return (
    <div
      ...
      style={{
        backgroundColor: section === 2 ? "#EDF4FF" : "white",
      }}
    >
      ...
      <div className="mt-5 mb-3.75">
        ...
        >
          <div className="flex gap-2 items-center">
            <img src="/Plan.svg" alt="camera" className="size-6.5" />
            <p className="text-[#0B0D10] text-[22px] font-bold">
              Choose your paln
            </p>
          </div>
          <div className="flex gap-1 items-center text-[#4E2FD2] text-[14px] ">
            <p className="font-medium  ">{plans ? 1 : 0} selected</p>
            <IoMdArrowDropup
              className="size-3 duration-300 transition"
              style={{ rotate: section === 2 ? "180deg" : "0deg" }}
            />
          </div>
        </div>
      </div>
      <div
        className={`overflow-hidden transition-all duration-300 ${section === 2 ? "max-h-250 opacity-100" : "max-h-0 opacity-0"}
            `}
      >
        {(isLoading || isPending) && (
          <div className="rounded-[10px] p-6 flex flex-col items-center justify-center gap-3">
            ...
            <p className="text-[#4E2FD2] font-medium">Loading plans...</p>
          </div>
        )}
